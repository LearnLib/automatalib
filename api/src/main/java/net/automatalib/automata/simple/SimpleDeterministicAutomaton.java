...
public interface SimpleDeterministicAutomaton<S, I> extends
	...
	/**
	 * Basic interface for integer abstractions of automata. In an integer abstraction,
	 * each state of an automaton is identified with an integer in the range {@code [0, size() - 1]}.
	 * A similar abstraction may be imposed on the input symbols, this is however not
	 * prescribed by this interface (see {@link StateIntAbstraction} and {@link FullIntAbstraction}).
	 * 
	 * @author Malte Isberner
	 */
	public static interface IntAbstraction {
		/**
		 * Representative for an invalid state. This is the value being returned by methods
		 * that would return {@code null} in their non-abstracted version. However,
		 ...
		 * of the range {@code [0, size() - 1]} is invalid, in particular all negative integers.
		 */
		 ...
		 * If the automaton has no initial state, {@link #INVALID_STATE} is returned.
		 ...
		 * {@link StateIDs} mapping obtained via {@link SimpleDeterministicAutomaton#stateIDs()}.
		 * 
		 * @author Malte Isberner
		 *
		 * @param <S> state type
	...
	/**
	 * Interface for {@link IntAbstraction integer abstractions} of an automaton that operate on
	 * non-abstracted input symbols (i.e., input symbols are of type {@code I}).
	 * 
	 * @author Malte Isberner
	 *
	 * @param <I> input symbol type
	 */
	public static interface StateIntAbstraction<I> extends IntAbstraction {
		/**
		 ...
		 * @return the integer representing the successor state, or {@link IntAbstraction#INVALID_STATE}
	...
	/**
	 * Interface for an {@link IntAbstraction integer abstraction} that abstracts both states and input symbols
	 * to integers. In addition to the modalities specified in {@link IntAbstraction}, this interface
	 * prescribes that input symbols are abstracted to integers in the range {@code [0, numInputs() - 1]}.
	 * 
	 * @author Malte Isberner
	 */
	public static interface FullIntAbstraction extends IntAbstraction {
		/**
		 ...
		 * @return the integer representing the target state, or {@link IntAbstraction#INVALID_STATE}
		 * if there is no successor state.
		 */
		public int getSuccessor(int state, int input);
		
		/**
		 * Retrieves the number of input symbols. This determines the valid range of input symbols, which
		 * is {@code [0, numInputs() - 1]}.
		...
		/**
		 * Base class implementing the default way of obtaining a {@link FullIntAbstraction}, i.e., building
		 * on top of a {@link StateIntAbstraction} and a mapping from integers to (concrete) input symbols.
		 * 
		 * @author Malte Isberner
		 *
		 * @param <I> input symbol type
		 * @param <A> state abstraction type
		 */
	...
	/**
	 * Retrieves a {@link FullIntAbstraction} of this automaton, using the given number of
	 * (abstract) inputs and the inputs mapping.
	 * 
	 * @param numInputs the number of inputs represented in the full abstraction
	 * @param symMapping the mapping from integers in the range {@code [0, numInputs - 1]} to
	 * input symbols.
	...
	/**
	 * Retrieves a {@link FullIntAbstraction} of this automaton, using the mapping induced by the given
	 * alphabet as the abstraction for the input symbols.
	 * <p>
	 * This method is provided for convenience. It is equivalent to calling
	 * {@code fullIntAbstraction(alphabet.size(), alphabet)}.
	 * 
	 * @param alphabet the alphabet inducing the abstraction
	 * @return a {@link FullIntAbstraction}
	 */
