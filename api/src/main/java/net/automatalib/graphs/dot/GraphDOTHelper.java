...
public interface GraphDOTHelper<N, E> {
     ...
     *         if writing to <tt>a</tt> throws.
     */
    void writePostamble(Mapping<N, String> identifiers, Appendable a) throws IOException;

    void getGlobalNodeProperties(Map<String, String> properties);

    void getGlobalEdgeProperties(Map<String, String> properties);

    /**
     * Retrieves the GraphVIZ properties for rendering a single node. Additionally, the return value allows to control
     * whether or not to omit this node from rendering. If <tt>false</tt> is returned, the node will not be rendered.
    ...
    /**
     * Retrieves the GraphVIZ properties for rendering a single edge. Additionally, the return value allows to control
     * whether or not to omit this edge from rendering. If <tt>false</tt> is returned, the edge will not be rendered.
     * Consequently, any modifications to the properties map will have no effect.
     * <p>
     * The properties are stored in the {@link Map} argument. Note that if an implementation of a base class is
