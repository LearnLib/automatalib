...
public class Minimizer<S,L> {
	 ...
	 * 
	 * @param <E> edge identifier class.
	 * @param graph the automaton interface.
	 * @return a {@link MinimizationResult} structure, containing the
	 * state partition.
	 */
	public final <E>
			MinimizationResult<S,L> performMinimization(UniversalGraph<S,E,?,L> graph, Collection<? extends S> initialNodes) {
		// Initialize the data structures (esp. state records) and build
		// the initial partition.
		Collection<Block<S,L>> initialBlocks
	...
	private void updateBlocks() {
		for(Block<S,L> block : splitBlocks) {
			...
			if(reuseBlock) {
				UnorderedCollection<State<S,L>> first
					= subBlockIt.next();
