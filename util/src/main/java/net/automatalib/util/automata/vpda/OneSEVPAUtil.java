...
public class OneSEVPAUtil {
	...
	public static class ReachResult<L, I> {
		...
		public ReachResult(final L terminateLoc, final List<L> reachableLocs, final RichArray<Word<I>> accessSequences) {
			...
			for (I callSym : alphabet.getCallSymbols()) {
				for (I returnSym : alphabet.getReturnSymbols()) {
					for (int i = 0; i < queuePtr; i++) {
						...
						if (result.get(succIdx) == null) {
							Word<I> succAs = computeAs ? result.get(sevpa.getLocationId(src))
									.append(callSym)
									.concat(currAs.append(returnSym)) : Word.epsilon();
							result.set(succIdx, succAs);
							if (terminatePred.test(succ)) {
								return new ReachResult<>(succ, reachable, result);
							}
						...
						if (src != curr) {
							...
							if (result.get(succIdx) == null) {
								final Word<I> succAs = computeAs ? currAs.append(callSym)
	...
	public static <L, I> RichArray<Word<I>> computeAccessSequences(final OneSEVPA<L, I> sevpa, final VPDAlphabet<I> alphabet) {
