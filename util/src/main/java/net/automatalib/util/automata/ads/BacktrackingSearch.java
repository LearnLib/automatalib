...
public class BacktrackingSearch {
	 ...
	 * See {@link BacktrackingSearch#computeOptimal(MealyMachine, Alphabet, Set, CostAggregator)}.
	 */
	...
	private static <S, I, O> Optional<ADSNode<S, I, O>> compute(final MealyMachine<S, I, ?, O> automaton,
		...
		while (!splittingWordCandidates.isEmpty()) {
			...
			final Map<S, S> currentToInitialMapping = node.getPartition()
					.stream()
					.collect(Collectors.toMap(x -> automaton.getSuccessor(x, prefix), Function.identity()));
			...
			for (final I i : input) {
				...
				// no splitting word
				else if (prefix.length() < maximumSplittingWordLength) {
	 ...
	 * @param input          the input alphabet of the automaton
	 * @param states         the set of states which should be distinguished by the computed ADS
	 * @param costAggregator the optimization function by which solutions should be pruned
	 * @param <S>            (hypothesis) state type
	 * @param <I>            input alphabet type
	 * @param <O>            output alphabet type
	 * @return {@code Optional.empty()} if there exists no ADS that distinguishes the given states, a valid ADS
