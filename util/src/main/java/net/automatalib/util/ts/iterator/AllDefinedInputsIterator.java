...
public final class AllDefinedInputsIterator<S, I> extends AbstractTwoLevelIterator<S, I, TransRef<S, I, ?>> {

    private final TransitionSystem<S, I, ?> ts;
    private final Iterable<? extends I> inputs;

    public AllDefinedInputsIterator(Iterator<? extends S> stateIt,
    ...
    protected TransRef<S, I, ?> combine(S state, I input) {
        return new TransRef<>(state, input, ts.getTransitions(state, input));
    }
}
