...
/**
 * This class provides several methods to initialize a {@link PaigeTarjan} partition refinement
 * data structure from common sources, e.g., automata.
 * <p>
 * The counterpart of this class is {@link PaigeTarjanExtractors}, which provides methods to translate
...
public class PaigeTarjanInitializers {
	...
	public static enum AutomatonInitialPartitioning {
		/**
		 * States are initially partitioned by their state property, i.e., states with the same
		 * state property are initially placed in the same partition class.
		 */
		BY_STATE_PROPERTY {
			@Override
		...
		/**
		 * States are initially partitioned by all of their transition properties, i.e., states
		 * with the same input symbol/transition property combinations are initially placed
		 * in the same partition class. Note that if transitions are missing, a {@code null}
		 * property is assumed.
		 */
		...
		/**
		 * States are initially partitioned by both their state properties and their transition
		 * properties. This mode can be regarded as a combination of {@link #BY_STATE_PROPERTY}
		 * and {@link #BY_TRANSITION_PROPERTIES}, resulting in the coarsest initial partition
		 * that refines both partitions obtained using the other modes.
		 */
		BY_FULL_SIGNATURE {
			@Override
			public IntFunction<Object> initialClassifier(
		...
		public abstract IntFunction<Object> initialClassifier(
				UniversalDeterministicAutomaton.FullIntAbstraction<?, ?, ?> automaton);
	...
	private static final class CompleteStateSignature {
		...
		public static CompleteStateSignature build(
				UniversalDeterministicAutomaton.FullIntAbstraction<?, ?, ?> automaton,
				int state) {
		...
		public static <S,I> CompleteStateSignature build(
		...
		public static CompleteStateSignature buildFromTransitions(
				UniversalDeterministicAutomaton.FullIntAbstraction<?, ?, ?> automaton,
				int state) {
			int numInputs = automaton.numInputs();
		...
		public static <S,I>
		CompleteStateSignature buildFromTransitions(
				UniversalDeterministicAutomaton<S, I, ?, ?, ?> automaton,
				Alphabet<I> alphabet,
				S state) {
			int numInputs = alphabet.size();
		...
		private static void fillTransitionProperties(
				UniversalDeterministicAutomaton.FullIntAbstraction<?, ?, ?> automaton,
				int state,
				Object[] properties) {
			int numInputs = automaton.numInputs();
		...
		private static <S,I> void fillTransitionProperties(
				UniversalDeterministicAutomaton<S, I, ?, ?, ?> automaton,
				Alphabet<I> alphabet,
				S state,
				Object[] properties) {
	 ...
	 * 
	 * @param pt the partition refinement data structure
	 * @param absAutomaton the abstraction of the input automaton
	 * @param ip the initial partitioning mode
	 * @param pruneUnreachable whether or not to prune unreachable states during initialization
	 */
	 ...
	 * function.
	 * <p>
	 * The return value of the {@code initialClassification} function can be any valid {@code Object}
	 * reference (even {@code null}). States are initially placed in the same partition class if
	 * their return values from applying {@code initialClassification} have the same hash code
	 * (determined according to {@link Objects#hashCode(Object)}) and are equal (determined according
	 * to {@link Objects#equals(Object, Object)}.
	 * 
	 * @param pt the partition refinement data structure
	 * @param absAutomaton the abstraction of the input automaton
	 * @param initialClassification the function determining the initial classification
	 * @param pruneUnreachable whether or not to prune unreachable states during initialization
	 */
	public static void initCompleteDeterministic(
			PaigeTarjan pt,
	...
	private static void initCompleteDeterministicPrune(
			PaigeTarjan pt,
			SimpleDeterministicAutomaton.FullIntAbstraction absAutomaton,
			IntFunction<?> initialClassification) {
	...
	private static void initCompleteDeterministicNoPrune(
			PaigeTarjan pt,
			SimpleDeterministicAutomaton.FullIntAbstraction absAutomaton,
			IntFunction<?> initialClassification) {
		int numStates = absAutomaton.size();
	 ...
	 * <p>
	 * This method can be used for automata with partially defined transition
	 * functions.
	 * 
	 * @param pt the partition refinement data structure
	 * @param automaton the input automaton
	 * @param inputs the input alphabet to consider, which also determines the
	 * mapping from {@code int}s to input symbols in the data structure
	 * @param initialClassification the initial classification function
	 * @param sinkClassification determines how a sink is being classified.
	 * @return the state ID mapping relating {@code int}s representing states in
	 ...
	 * @param inputs the input alphabet to consider, which also determines the
	 * mapping from {@code int}s to input symbols in the data structure
	 * @param initialClassification the initial classification predicate
	 * @param sinkClassification determines how a sink is being classified.
	 * @return the state ID mapping relating {@code int}s representing states in
	 * the partition refinement data structure to states of the automaton, and
	 * vice versa
