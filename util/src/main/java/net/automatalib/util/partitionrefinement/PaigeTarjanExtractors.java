...
/**
 * This class provides methods for translating the result of a {@link PaigeTarjan} coarsest
 * stable partition computation into several common, more usable forms such as automata.
 * <p>
 * Most of the methods defined in this class expect the partition data to be in a certain
 * form, and moreover may require additional information, both of which is provided by
 * corresponding methods defined in {@link PaigeTarjanInitializers}.
 * 
...
public class PaigeTarjanExtractors {
	 ...
	 * </li>
	 * <li>
	 * {@link PaigeTarjanInitializers#initDeterministic(PaigeTarjan, DeterministicAutomaton, Alphabet, java.util.function.Predicate, boolean)}
	 * </li>
	 * </ul>
	 * <p>
	 * Both the {@code spExtractor} and the {@code tpExtractor} can be {@code null}, in which
	 * case they are replaced by a function always returning {@code null}.
	 * 
	 * @param pt the partition refinement data structure, after computing the coarsest stable
	 * partition
	 * @param creator an {@link AutomatonCreator} for creating the resulting automaton
	 * @param inputs the input alphabet to use
	 * @param original the original automaton on which the partition was computed
	 * @param origIds the {@link StateIDs} that translate the {@code int}s from the {@link PaigeTarjan}
	 * to states of {@code original} (e.g., obtained as the result from
	 * {@link PaigeTarjanInitializers#initDeterministic(PaigeTarjan, DeterministicAutomaton, Alphabet, java.util.function.Predicate, boolean)}
	 * @param spExtractor the state property extractor, or {@code null}
	 * @param tpExtractor the transition property extractor, or {@code null}
	 * @param pruneUnreachable {@code true} if unreachable states should be pruned
	 * during construction, {@code false} otherwise
	 * @return an automaton created using the specified creator, over the specified
	 * input alphabet, and reflecting the partition data of the specified
	 * {@link PaigeTarjan} object
	 */
	...
	A toDeterministic(
		...
		if (pruneUnreachable) {
			return toDeterministicPruned(pt, creator, inputs, original, origIds, spExtractor, tpExtractor);
		}
		return toDeterministicUnpruned(pt, creator, inputs, original, origIds, spExtractor, tpExtractor);
	}
	
	private static <S1,S2,I,T1,T2,SP,TP,A extends MutableDeterministic<S2, I, T2, SP, TP>>
	A toDeterministicUnpruned(
			PaigeTarjan pt,
			AutomatonCreator<A, I> creator, Alphabet<I> inputs,
			DeterministicAutomaton<S1, I, T1> original,
			StateIDs<S1> origIds,
	 ...
	 * <ul>
	 * <li>
	 * {@link PaigeTarjanInitializers#initCompleteDeterministic(PaigeTarjan, net.automatalib.automata.simple.SimpleDeterministicAutomaton.FullIntAbstraction, IntFunction, boolean)}
	 * </li>
	 * <li>
	 * {@link PaigeTarjanInitializers#initCompleteDeterministic(PaigeTarjan, net.automatalib.automata.UniversalDeterministicAutomaton.FullIntAbstraction, net.automatalib.util.partitionrefinement.PaigeTarjanInitializers.AutomatonInitialPartitioning, boolean)} 
	 * </li>
	 * </ul>
	 * <p>
	 * Both the {@code spExtractor} and the {@code tpExtractor} can be {@code null}, in which
	 ...
	 * @param creator an {@link AutomatonCreator} for creating the resulting automaton
	 * @param inputs the input alphabet to use
	 * @param absOriginal the abstraction of the original automaton that was used to build the partition
	 ...
	 * {@link PaigeTarjan} object
	 */
	public static <I,T,SP,TP,A extends MutableDeterministic<?, I, ?, SP, TP>>
	A toDeterministic(
			PaigeTarjan pt,
			AutomatonCreator<A, I> creator, Alphabet<I> inputs,
	...
	private static <I,T,SP,TP,A extends MutableDeterministic<?, I, ?, SP, TP>>
	A toDeterministicUnpruned(
			PaigeTarjan pt,
		...
		A result = creator.createAutomaton(inputs, numBlocks);
		MutableDeterministic.FullIntAbstraction<?, SP, TP> resultAbs
			= result.fullIntAbstraction(inputs);
